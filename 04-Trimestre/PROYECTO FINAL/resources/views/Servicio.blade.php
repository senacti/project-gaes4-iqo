...
<div class="logo-container">
    <img src="{{ asset('images/logoIQO.png') }}" alt="Logo" class="logo-image" style="width: 300px; height: auto;">
</div>
...
<div class="btn-group" role="group">
    <a href="{{ url('/AdminServiciosTecnicos') }}" class="btn btn-primary">Volver a AdminServiciosTecnicos</a>
    <a href="{{ url('/TecnicoConsultarServiciosTecnicos') }}" class="btn btn-primary">Volver a TecnicoServiciosTecnicos</a>
    <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#modalRegistrar">Añadir Producto</button>
    <a href="{{ url('/pdf3') }}" class="btn btn-success" target="_blank">PDF</a>
</div>
         ...
         <table class="table table-striped table-bordered table-hover">
            ...
            <tbody class="table-group-divider">
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalEditar{{$item->ID_SERVICIO}}">Modificar</button>
                        <button data-bs-href="{{ route('Crud_Servicios.delete', $item->ID_SERVICIO) }}" onclick="return res()" type="button" class="btn btn-danger">Eliminar</button>
                    </td>
...
<div class="modal fade" id="modalRegistrar" tabindex="-1" aria-labelledby="modalRegistrarLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('Crud_Servicios.Create') }}">
                    @csrf
                    <div class="mb-3">
   ...
   @foreach ($datos as $item)
   {{-- modal editar --}}
    <div class="modal fade" id="modalEditar{{$item->ID_SERVICIO}}" tabindex="-1" aria-labelledby="modalEditarLabel{{$item->ID_SERVICIO}}" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="modalEditarLabel{{$item->ID_SERVICIO}}">Modificar</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{ route('Crud_Servicios.update') }}">
                                 @csrf

                        <div class="mb-3">
                            <label for="txtCodigo" class="form-label">ID SERVICIO</label>
                            <input type="text" class="form-control" id="txtCodigo" aria-describedby="emailHelp" name="txtID_SERVICIO" value="{{$item->ID_SERVICIO}}" readonly>
                        </div>
                        <div class="mb-3">
                            <label for="txtNombreP" class="form-label">Descripcion Servicio</label>
                            <input type="text" class="form-control" id="txtNombreP" name="txtDESCRIPCIONSERVICIO">
